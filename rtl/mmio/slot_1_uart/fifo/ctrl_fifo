...
architecture arch of fifo_ctrl is
    ...
    signal w_ptr_reg, w_ptr_next : std_logic_vector(ADDR_WIDTH-1 downto 0);
    signal r_ptr_reg, r_ptr_next : std_logic_vector(ADDR_WIDTH-1 downto 0);
...
begin
    -- rgister for read and write pointers
    process(clk, reset)
    begin
        if reset = '1' then
            w_ptr_reg <= (others => '0');
    ...
    -- usccessive pointers values
    w_ptr_succ <= std_logic_vector(unsigned(w_ptr_reg) + 1);
    r_ptr_succ <= std_logic_vector(unsigned(r_ptr_reg) + 1);

    -- next state lofic for read and write pointers
    wr_op <= wr & rd;
